...
myfont = fm.FontProperties(fname='/home/jin/test/datasite/msyhl.ttc')


def stackbar(df,  # 传入数据df，应该是一个行索引为date的时间序列面板数据
...
def prepare_chart(df,  # 输入经过pivoted方法透视过的df，不是原始df
    ...
    if chart_type == 'bar_total_trend':
        ...
        chart = stackbar(df=df_abs, df_gr=df_gr)  # 调用stackbar方法生成Pyecharts图表对象
